...
PRODUCT_NAME_KEYS = [
    # Standard GHS / OSHA / REACH labels
    'Product name', 'Product Name', 'GHS product identifier',
    'Identification of the substance', 'Product identifier',
    'Trade name', 'Trade Name', 'Chemical name', 'Chemical Name',
    # Expanded / vendor-specific labels
    'SRM Name', 'SRM name',                          # NIST Standard Reference Materials
    'Material Name', 'Material name',
    'Substance name', 'Substance Name',
    'Name of substance', 'Name of Substance',
    'Chemical product name', 'Chemical Product Name',
...
# Regex that matches catalog / article / part number markers.
# Everything from the first match onward is stripped from the product name.
_CAT_NO_STRIP_RE = re.compile(
    r'[\s,;]+(?:Cat(?:alogue|alog)?\.?\s*[Nn]o|'
    r'Part\s*[Nn]o|Article\s*[Nn]o|Item\s*[Nn]o|'
    r'Order\s*[Nn]o|Product\s*[Nn]umber|'
    r'Catalog(?:ue)?\s+[Nn]umber|Ref\.?\s*[Nn]o|'
    r'P/?N)\b.*$',
    re.IGNORECASE | re.DOTALL,
)

# Titles that are known to hold the product name but use non-standard wording
_PRODUCT_NAME_TITLE_RE = re.compile(
    r'\b(?:product|substance|material|chemical|trade|srm|item|article)\b.*'
    r'\b(?:name|identifier|designation|description)\b|'
    r'\b(?:name|identifier|designation)\b.*'
    r'\b(?:product|substance|material|chemical)\b',
    re.IGNORECASE,
)
...
def _clean_product_name(raw: str) -> str:
    """
    Strip catalog numbers and other trailing cruft from a raw product-name string.

    Examples
    --------
    "Acetone Cat No. A9-4; A9-20; ..."  → "Acetone"
    "Acetone; dimethyl ketone"           → "Acetone; dimethyl ketone"   (synonyms kept)
    "Ethanol Solution 96%"               → "Ethanol Solution 96%"       (unchanged)
    """
    if not raw:
        return raw
    cleaned = _CAT_NO_STRIP_RE.sub('', raw).strip().strip(',;').strip()
    return cleaned if cleaned else raw


# N/A-like content that should never be treated as a product name
_NA_CONTENT_RE = re.compile(
    r'^[\s.]*(?:not\s+applicable|n/?a\.?|none|not\s+available|not\s+assigned|'
    r'not\s+established|unknown|—|\-|n\.a\.)\s*\.?\s*$',
    re.IGNORECASE,
)

# Sub-labels within a merged product-identifier block that ARE the product name
_SUB_PRODUCT_NAME_RE = re.compile(
    r'\b(?:SRM\s+Name|Trade\s+[Nn]ame|Chemical\s+[Nn]ame|Product\s+[Nn]ame|'
    r'Material\s+[Nn]ame|Substance\s+[Nn]ame|Common\s+[Nn]ame)'
    r'[\s:]+([^\n;]{2,80})',
    re.IGNORECASE,
)


def _is_valid_product_name(content: str) -> bool:
    """Return True if content looks like a real product name (not N/A, too short, etc.)"""
    if not content or len(content.strip()) < 2:
        return False
    return not _NA_CONTENT_RE.match(content.strip())


def _extract_sub_product_name(merged_content: str) -> str | None:
    """
    When a product-identifier block has merged sub-fields, e.g.:
        "SRM Number: 4330C\nSRM Name: Plutonium-239 Radioactivity Standard\n"
        "Other Means of Identification: Not applicable."
    Extract the name part from within the block.
    """
    m = _SUB_PRODUCT_NAME_RE.search(merged_content)
    if m:
        candidate = _clean_product_name(m.group(1).strip())
...
def extract_product_name(sections: list, layout_lines: list[LayoutLine]) -> str:
    # Flexible Section 1 match — handles "1", "1.", "1 " etc.
    sec1 = next(
        (s for s in sections
         if str(s.get('section_number', '')).rstrip('. ').strip() == '1'),
        None,
    )
    if sec1:
        # ── Pass 1: exact match against known product-name label keys ──────────
        for sub in sec1['subsections']:
            title       = sub['title'].strip()
            raw_content = sub.get('content', '').strip().lstrip(': ')

            for key in PRODUCT_NAME_KEYS:
                if title.lower() == key.lower():
                    # If the content looks like a merged block, try to extract
                    # the actual product name sub-field first
                    sub_name = _extract_sub_product_name(raw_content)
                    if sub_name:
                        return sub_name
                    # Otherwise clean and validate the whole content
                    content = _clean_product_name(raw_content)
                    if _is_valid_product_name(content):
                        return content

        # ── Pass 2: relaxed keyword match on the subsection title ─────────────
        # Catches unusual labels like "SRM Designation", "Substance/Trade Name".
        # Explicitly excludes "Other Means of Identification" patterns.
        for sub in sec1['subsections']:
            title       = sub['title'].strip()
            raw_content = sub.get('content', '').strip().lstrip(': ')
            # Skip titles that are clearly NOT product names
            if re.search(r'\bother\s+means\b|\bidentification\b.*\bother\b', title, re.IGNORECASE):
                continue
            if _PRODUCT_NAME_TITLE_RE.search(title):
                sub_name = _extract_sub_product_name(raw_content)
                if sub_name:
                    return sub_name
                content = _clean_product_name(raw_content)
    ...
    # ── Fallback: first large bold line from the layout ───────────────────────
    for ll in layout_lines:
        if ll.size >= 10.0 and ll.is_bold:
            ...
            if (3 < len(t) <= 80
                    ...
                    and not _NA_CONTENT_RE.match(t)
                    and re.search(r'[A-Za-z]{2,}', t)):
                return _clean_product_name(t)
    return ''
...
if __name__ == '__main__':
    ...
    for name, path in FILES.items():
        ...
        print(json.dumps(all_output[name], indent=2))
    ...
    for name, data in all_output.items():
        json_path = out_dir / f'{name}_v2.json'
...
{table_rows_html}    </tbody>
        ...
        html_path = out_dir / f'sds_{name}_v2.html'
